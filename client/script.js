...
sayHelloButton.addEventListener('mouseenter', changeButtonColor)
// PROBLEM 3
/*
...
const ohMy = () => {
    ...
    .then(res => {
        for (let i = 0; i < res.data.length; i++){
            ...
            document.querySelector('body').append(newPTag)
        }
    })
}
...
// CODE HERE
const queryFunction = () => {
    axios.get('http://localhost:3000/query-test?age=30&name=zach')
    .then(res => {
        console.log(res)
    })
}
...
/*
    ...
    1: Send no queries on the URL -- what happened? 
        The backend has logic to send back the result: 'You sent an empty query!'

    2: Send more than 1 query on the URL -- what happened? 
        result: console.log('You sent more than 1 query!)
*/
